...
7. 添加一个普通组并记录重量、次数、RPE。
8. 添加一个 3 动作超级组并记录两轮。
9. 保存训练。
10. 从最近训练筛选一条同类记录并复制一份，修改重量后再保存。
11. 进入训练详情，编辑训练信息、编辑某一组、删除一组测试数据。
12. 打开“档案”，点击“重算”，确认最大重量、总组数和容量更新。
13. 确认档案页近 7 天 / 近 30 天摘要正确，PR 关注最多展示 3 个动作。
14. 导出一次 JSON 备份，确认文件包含训练、组明细、统计和目标。
15. 两个人分别扫码测试，确认只能看到自己的训练、目标和档案。
